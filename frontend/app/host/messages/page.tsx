...
export default function HostMessagesPage() {
  const { currentUser, viewMode, loading: authLoading } = useAuth();
  ...
  useEffect(() => {
    ...
    loadConversations();
  }, [currentUser, viewMode, router, authLoading]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-0 sm:px-6">
      <div className="flex h-[calc(100vh-80px)] border-x border-gray-100">
        {/* Left: Conversation list */}
        <div className="w-full sm:w-96 border-r border-gray-100 flex flex-col shrink-0">
          ...
          <div className="flex border-b border-gray-100 px-4">
            <button
              ...
              className={`px-4 py-3 text-sm font-medium transition-colors ${activeTab === "all" ? "border-b-2 border-black" : "text-gray-400 hover:text-gray-600"}`}
            >
              All
            </button>
            <button
              onClick={() => setActiveTab("unread")}
              className={`px-4 py-3 text-sm font-medium transition-colors ${activeTab === "unread" ? "border-b-2 border-black" : "text-gray-400 hover:text-gray-600"}`}
            >
              Unread
            </button>
          </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 bg-gray-50/30">
                {messagesLoading ? (
                  <div className="flex items-center justify-center h-full">
                    <div className="w-6 h-6 border-2 border-gray-200 border-t-[#E51E5B] rounded-full animate-spin" />
                  </div>
                ) : messages.length === 0 ? (
                      ...
                      <div key={msg.id} className={`flex gap-4 mb-6 ${isMe ? "justify-end" : ""}`}>
                        ...
                        <div className={`flex flex-col gap-1 max-w-[80%] ${isMe ? "items-end" : ""}`}>
                          ...
                          <div className={`px-4 py-3 rounded-2xl text-sm ${
                            isMe
                              ? "bg-[#E51E5B] text-white rounded-tr-none"
                        ...
                        {isMe && (
                          <div className="w-10 h-10 rounded-full bg-[#E51E5B] text-white flex items-center justify-center font-bold shrink-0">
              ...
              <div className="px-6 py-4 border-t border-gray-100">
                <div className="flex gap-3">
                  <input
                    ...
                    className="flex-1 border border-gray-200 rounded-full px-4 py-3 text-sm focus:outline-none focus:border-gray-400 transition-colors"
                  />
                  <button
                    ...
                    className={`px-6 py-3 rounded-full text-sm font-medium transition-colors ${
                      replyText.trim() && !sending
                        ? "bg-[#E51E5B] text-white hover:bg-[#d11650]"
